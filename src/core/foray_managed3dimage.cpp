...
namespace foray::core {
    Managed3dImage::CreateInfo::CreateInfo() :
            ImageCI{
            ...
            ImageViewCI{
                    ...
                    .subresourceRange = VkImageSubresourceRange{
                            .aspectMask = VkImageAspectFlagBits::VK_IMAGE_ASPECT_COLOR_BIT, .levelCount = 1U, .layerCount = 1U
                    }
            },
            AllocCI{
    ...
    Managed3dImage::CreateInfo::CreateInfo(VkImageUsageFlags usage, VkFormat format, const VkExtent3D &extent, std::string_view name) : CreateInfo() {
        ImageCI.usage = usage;
    ...
    void Managed3dImage::Create(Context *context, const CreateInfo &createInfo) {
        mContext = context;
    ...
    void Managed3dImage::Resize(const VkExtent3D &newExtent) {
    ...
    void Managed3dImage::Create(Context *context, VkImageUsageFlags usage, VkFormat format, const VkExtent3D &extent, std::string_view name) {
        CreateInfo createInfo(usage, format, extent, name);

        Create(context, createInfo);
    }

    void Managed3dImage::TransitionLayout(Managed3dImage::QuickTransition &quickTransition, VkCommandBuffer commandBuffer) {
        VkImageMemoryBarrier barrier{
                ...
                .image               = mImage,
                .subresourceRange = VkImageSubresourceRange{
                        .aspectMask = quickTransition.AspectMask,
    ...
    void Managed3dImage::WriteDeviceLocalData(const void *data, size_t size, VkImageLayout layoutAfterWrite, VkBufferImageCopy &imageCopy) {
    ...
    void
    Managed3dImage::WriteDeviceLocalData(HostSyncCommandBuffer &cmdBuffer, const void *data, size_t size, VkImageLayout layoutAfterWrite, VkBufferImageCopy &imageCopy) {
    ...
    void Managed3dImage::WriteDeviceLocalData(const void *data, size_t size, VkImageLayout layoutAfterWrite) {
        HostSyncCommandBuffer cmdBuffer;
        cmdBuffer.Create(mContext);
        WriteDeviceLocalData(cmdBuffer, data, size, layoutAfterWrite);
    }

    void Managed3dImage::WriteDeviceLocalData(HostSyncCommandBuffer &cmdBuffer, const void *data, size_t size, VkImageLayout layoutAfterWrite) {
    ...
    void Managed3dImage::CheckImageFormatSupport(const CreateInfo &createInfo) {
        VkImageFormatProperties props{};
        // check if image format together with required flags and usage is supported.
        AssertVkResult(
                vkGetPhysicalDeviceImageFormatProperties(mContext->PhysicalDevice(), createInfo.ImageCI.format, createInfo.ImageCI.imageType, createInfo.ImageCI.tiling,
                                                         createInfo.ImageCI.usage, createInfo.ImageCI.flags, &props));
    }

    void Managed3dImage::SetName(std::string_view name) {
        mName = name;
#if FORAY_DEBUG
        if(mAllocation)
        {
    ...
    void Managed3dImage::UpdateDebugNames() {
        {  // Image
