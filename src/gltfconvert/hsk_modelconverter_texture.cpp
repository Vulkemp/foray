        ...
        void lLoadTexturesThreadInstance(MultithreadLambdaArgs args)
        {
            for(int32_t texIndex = args.ThreadIndex; texIndex < args.GltfModel.textures.size(); texIndex += args.ThreadCount)
            {
                try
                {
                    ...
                    SampledTexture& sampledTexture = args.Textures.GetTextures()[args.BaseTexIndex + texIndex];
                    sampledTexture.Image           = std::make_unique<ManagedImage>();

                    const unsigned char* buffer     = nullptr;
                    VkDeviceSize         bufferSize = 0;

                    ImageLoader<VkFormat::VK_FORMAT_R8G8B8A8_UNORM> imageLoader;
                    ...
                    {  // Any GPU action for the moment is single threaded
                        ...
                        {
                            hsk::ModelConverter::sTranslateSampler(args.GltfModel.samplers[gltfTexture.sampler], samplerCI);
                        }
