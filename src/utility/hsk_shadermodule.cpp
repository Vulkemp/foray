...
namespace hsk {
    ...
    void ShaderModule::LoadFromSpirv(const VkContext* context, std::string relativeSpirvPath)
    {
        ...
        {
            throw Exception("{} Could not open shader file: {}", __FUNCTION__, mAbsoluteSpirvPath);
        }
    }
    ...
    ShaderModule::operator VkShaderModule() const
    {
        if(mShaderModule == nullptr)
        {
            throw Exception("{} Attempt to access VkShaderModule but shader module is nullptr: {}", __FUNCTION__);
        }
