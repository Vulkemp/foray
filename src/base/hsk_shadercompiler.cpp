...
#else
    inline SPV_STR PathToString(const fs::path& path) { return path.string(); }
...
#define SPIRV_FILEENDING std::string(".spv")
    ...
    bool ShaderCompiler::CompileShaderFile(fs::path sourceDir, fs::path shaderFilePath)
    {
        ...
        if(!compileResult)
        {
            logger()->error("Failed to compile: {} ", shaderFileInfo.mSourcePathFull.string());
            if(mThrowException)
            {
                throw hsk::Exception("Shader compile failed for file: {}", shaderFileInfo.mSourcePathFull.string());
            }
        }
    ...
    bool ShaderCompiler::StartsWith(const SPV_STR& str, const SPV_STR& prefix) { return str.size() >= prefix.size() && 0 == str.compare(0, prefix.size(), prefix); }
