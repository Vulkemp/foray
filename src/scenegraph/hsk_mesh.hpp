...
namespace hsk {
    ...
    struct Primitive
    {
        ...
        inline Primitive() {}
        inline Primitive(EType type, uint32_t first, uint32_t count, int32_t materialIndex) : Type(type), First(first), VertexOrIndexCount(count), MaterialIndex(materialIndex) {}
