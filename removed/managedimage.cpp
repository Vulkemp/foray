...
namespace foray::core {
    ManagedImageOld::CreateInfo::CreateInfo()
    ...
    ManagedImageOld::CreateInfo::CreateInfo(VkImageUsageFlags usage, vk::Format format, const VkExtent2D& extent, std::string_view name) : CreateInfo()
    {
        ImageCI.usage      = usage;
        ImageCI.format     = format;
        ImageViewCI.format = format;
        ImageCI.extent     = vk::Extent3D{.width = extent.width, .height = extent.height, .depth = 1};
        Name               = name;
    }

    ManagedImageOld::ManagedImageOld(Context* context, const CreateInfo& createInfo) : VulkanResource<VK_OBJECT_TYPE_IMAGE>("Unnamed Buffer")
    {
        mContext    = context;
    ...
    ManagedImageOld::CreateInfo ManagedImageOld::GetInfoForResize(const VkExtent2D& newExtent)
    {
        Assert(Exists(), "Attempted to resize image before initial creation!");
        mCreateInfo.ImageCI.extent = vk::Extent3D{.width = newExtent.width, .height = newExtent.height, .depth = 1};
        return mCreateInfo;
    }
    ManagedImageOld::CreateInfo ManagedImageOld::GetInfoForResize(const vk::Extent3D& newExtent)
    {
        Assert(Exists(), "Attempted to resize image before initial creation!");
    ...
    ManagedImageOld::ManagedImageOld(Context* context, VkImageUsageFlags usage, vk::Format format, const VkExtent2D& extent, std::string_view name)
        : ManagedImageOld(context, CreateInfo(usage, format, extent, name))
    {
    }

    void ManagedImageOld::TransitionLayout(ManagedImageOld::QuickTransition& quickTransition, VkCommandBuffer commandBuffer)
    {
    ...
    void ManagedImageOld::WriteDeviceLocalData(const void* data, size_t size, VkImageLayout layoutAfterWrite, VkBufferImageCopy& imageCopy)
    {
        HostSyncCommandBuffer cmdBuffer(mContext);
        WriteDeviceLocalData(cmdBuffer, data, size, layoutAfterWrite, imageCopy);
    }
    void ManagedImageOld::WriteDeviceLocalData(HostSyncCommandBuffer& cmdBuffer, const void* data, size_t size, VkImageLayout layoutAfterWrite, VkBufferImageCopy& imageCopy)
    {
    ...
    void ManagedImageOld::WriteDeviceLocalData(const void* data, size_t size, VkImageLayout layoutAfterWrite)
    {
        HostSyncCommandBuffer cmdBuffer(mContext);
        WriteDeviceLocalData(cmdBuffer, data, size, layoutAfterWrite);
    }
    void ManagedImageOld::WriteDeviceLocalData(HostSyncCommandBuffer& cmdBuffer, const void* data, size_t size, VkImageLayout layoutAfterWrite)
    {
    ...
    ManagedImageOld::~ManagedImageOld()
    {
        if(mAllocation)
        {
            if(!!mImageView)
    ...
    void ManagedImageOld::CheckImageFormatSupport(const CreateInfo& createInfo)
    {
    ...
    void ManagedImageOld::SetName(std::string_view name)
    {
        mName = name;
#if FORAY_DEBUG
        if(mAllocation)
    ...
    void ManagedImageOld::UpdateDebugNames()
    {
        {  // Image
        ...
        {  // Image View
            ...
            SetVulkanObjectName(mContext, vk::ObjectType::VK_OBJECT_TYPE_IMAGE_VIEW, mImageView, debugName);
        }
    }


    void Local_ManagedImageOld::New(core::Context* context, const ManagedImageOld::CreateInfo& createInfo)
    {
        Local<ManagedImageOld>::New(context, createInfo);
    }

    void Local_ManagedImageOld::Resize(VkExtent2D extent)
    {
        Assert(Exists());
        core::Context*           context = GetRef().GetContext();
        ManagedImageOld::CreateInfo ci      = GetRef().GetInfoForResize(extent);
        New(context, ci);
    }

    void Local_ManagedImageOld::Resize(vk::Extent3D extent)
    {
        Assert(Exists());
        core::Context*           context = GetRef().GetContext();
        ManagedImageOld::CreateInfo ci      = GetRef().GetInfoForResize(extent);
